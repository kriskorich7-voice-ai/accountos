...
// Calls the Anthropic proxy with claude-sonnet-4-6. Throws if unavailable so the
// caller can fall back to the local knowledge base (keeps the demo working offline).
...
export function localAnswer(q) {
  ...
  if (t.includes('expansion') || t.includes('opportunit') || t.includes('biggest'))
    return 'The biggest opportunity is Conversational AI in Customer Service: $780K–$1.2M at 87% confidence. Acme already runs high-volume streaming speech in an established CS operation, and usage growth signals readiness. Next step: technical discovery with Daniel Rodriguez on Voice Agent API, Flux STT, and Aura-2 TTS. Sales Voice ($250K–$400K) and Marketing ($180K–$300K) follow.';
